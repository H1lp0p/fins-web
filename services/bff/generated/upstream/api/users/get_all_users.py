...
def _parse_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> BffErrorBody | list[UserDto] | None:
...
def _build_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> Response[BffErrorBody | list[UserDto]]:
    return Response(
...
) -> Response[BffErrorBody | list[UserDto]]:
    """ 
    Raises:
...
) -> BffErrorBody | list[UserDto] | None:
    """ 
    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.
